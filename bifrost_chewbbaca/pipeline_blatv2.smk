...
rule blat_gene_call:
    ...
    params:
        samplecomponent_ref_json = samplecomponent.to_reference().json,
        chewbbaca_blatdb = f"{os.environ['BIFROST_CG_MLST_DIR']}/blatdb/",
    output:
    ...
    script:
        os.path.join(os.path.dirname(workflow.snakefile), "rule__blat_genecallv2.py")
...
rule run_chewbbaca_on_genome:
    ...
    input:
        rules.check_requirements.output.check_file,
        rules.blat_gene_call.output.gene_call_done,
        #genome = f"{sample['categories']['contigs']['summary']['data']}"
        genome = rules.blat_gene_call.output.gene_calls
    output:
