...
rule_name = "mmseqs_gene_call"
rule mmseqs_gene_call:
    message:
        f"Running step:{rule_name}"
    log:
        out_file = f"{component['name']}/log/{rule_name}.out.log",
    ...
    params:
        ...
        chewbbaca_mmseqs_db = f"{os.path.join(os.path.dirname(workflow.snakefile), f'all_{species_sp}_loci_db')}",
    output:
        gene_call_results = directory(f"{component['name']}/mmseqs_gene_call_results"),
        gene_calls = f"{component['name']}/mmseqs_gene_call_results/gene_calls.fa",
        gene_call_done = f"{component['name']}/mmseqs_gene_call_done"
    message:
        f"Running step:{rule_name}, outputs: {component['name']}/mmseqs_gene_call_results"
    script:
        os.path.join(os.path.dirname(workflow.snakefile), "rule__mmseqs_genecall.py")
...
rule run_chewbbaca_on_genome:
    ...
    input:
        rules.check_requirements.output.check_file,
        rules.mmseqs_gene_call.output.gene_call_done,
        #genome = f"{sample['categories']['contigs']['summary']['data']}"
        genome = rules.mmseqs_gene_call.output.gene_calls
    output:
