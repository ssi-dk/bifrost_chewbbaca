...
def run_blastn_and_parse(query_fa, db, assembly_sequences,log):
    ...
    query_contigs = SeqIO.parse(query_fa, "fasta")
    new_query_ORF = []
    ...
    for contig in query_contigs:
        ...
        for nt_start in range(contig_len): #shift nucleotides by one when searching for start codon
            if contig_seq[nt_start:nt_start+3] == start_codon:
                for nt_end in range(nt_start + 3, contig_len, 3): #once start codon is located shift search by 3 -> full codon search
                    codon = contig_seq[nt_end:nt_end+3]
                    if codon in stop_codons:
                        start = nt_start + 1  # 1-based indexing
                        end = nt_end + 3
                        orf_seq = contig_seq[start-1:end]  # Extract ORF sequence
                        orf_id = f"{contig_id}_ORF{len(new_query_ORF)+1}_{start}_{end}"
                        new_query_ORF.append(SeqRecord(Seq(orf_seq), id=orf_id, description=""))
                        break

    # Create an in-memory file-like object for the ORFs
    orf_output = StringIO()
    SeqIO.write(new_query_ORF, orf_output, "fasta")
    orf_output.seek(0)  # Reset the StringIO object to the beginning
