...
parser.add_argument(
    "--address",
    "-a",
    default="https://enterobase.warwick.ac.uk/schemes/Yersinia.cgMLSTv1/",
    help="URL containing enterobase fasta links",
)
parser.add_argument(
    "--output_folder",
    "-o",
    default="/srv/data/MPV/LEBC/test_download",
    help="scheme output folder",
)
...
def get_html(address):
    ...
    html_output = subprocess.check_output(
        f"curl -k -X GET --header 'Accept: application/json' --user '{API_TOKEN}:' {address}",
        shell=True,
    )
    soup = bs4.BeautifulSoup(html_output, features="html.parser")
    links_html = soup.find_all("a")
    links = [
        os.path.join(address, i["href"])
        for i in links_html
        if re.match(".*fasta.gz", i["href"])
    ]
    return links
